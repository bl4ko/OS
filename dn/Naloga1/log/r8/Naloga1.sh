...
preveri_uporanika () {
    ...
    do
        ...
        if cat /etc/passwd | grep "^$uporabnik" | cut -d: -f6 | xargs ls &> /dev/null
        then
            echo -n "obstaja "
        else
            echo -n "ne-obstaja "
...
izlusci () {
    ...
    for arg in "$@"
    do
        cat $arg | tee -a sum > /dev/null
    done
    sed -e '/^$/d' -e '/^#/d' sum | cut -d: -f2 | sort | uniq -c | sort -nr | cat -n
    rm sum
}
...
tocke () {
    # Preverimo robne Pogoje za vpisno
    # * Ce se vrstica zacne z zankom # potem ignoriramo
    # * Ce je tip enak crki p ali P, potem naj se sestevek prepolovi
    # * Ce sta 3. in 4. znak vpisne enaka 14, potem naj se sestevku pristeje nakljucno stevilo od 1 do 5 (seme RANDOM=42) 
    # * Ce sta ipzpolnjena oba zgornja pogoja: P in 14 potem naj se uposteva le pogoj P
    # * Koncni sestevek naj nebo vecji od 50
    # * Nakoncu se stevilo vseh studentov in povprecno stevilo tock
    # --------- Primer -----------------------------------------
    # Input:
    # 99131234 22 18 16
    # 99143244 1 4 9
    # 99136784 14 12 1 P
    # 99143148 3 2 11 p
    # Output:./Naloga1.sh tocke < tocke.txt
    # 99131234: 50
    # 99143244: 16
    # 99136784: 13
    # 99143148: 8
    ...
    # ker se vhod nekonca z '\n' ob branju zadne vrstice read vrne konec
